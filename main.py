...
# Flow: Upload image → OCR (default models) → show text + annotated image → TTS (pyttsx3)
# Notes: Requires packages.txt: `espeak-ng` for TTS on Linux (Streamlit Cloud)
...
def draw_ocr_simple(np_img: np.ndarray, boxes: List[List[List[float]]], txts: Optional[List[str]] = None) -> Image.Image:
    """Draw quadrilateral boxes and optional text labels on an image (no external deps)."""
...
# ---------------- Normalize PaddleOCR outputs (3.x + fallbacks) ----------------
def normalize_paddle_result(result: Any) -> Tuple[List[List[List[float]]], List[str], List[float]]:
    """Normalize PaddleOCR outputs to (boxes, texts, scores) across versions."""
            ...
            for det in dets:
                try:
                    if isinstance(det, dict):
                        ...
                        if box:
                            boxes.append(box); txts.append(str(text)); scores.append(float(score))
                except Exception:
                    continue
    return boxes, txts, scores
...
# ---------------- Cache OCR (3.x constructor: minimal, no deprecated args) ----------------
...
def run_ocr(img: Image.Image, lang: str) -> Tuple[str, Optional[Image.Image]]:
    ...
    def _infer(arr: np.ndarray):
        # PaddleOCR 3.x: call instance directly; fallback to .ocr(arr)
        try:
            return ocr(arr)
        except Exception:
            try:
    ...
    # If empty, try cleaned and 4 rotations, and latin fallback if not already latin
    def is_empty(res):
    ...
    # Optional language fallback for better Latin OCR
    if is_empty(raw) and lang != "latin":
        ocr_latin = load_ocr("latin")
        raw = _infer(np_img) or _infer(np.array(img.convert("RGB"))) or _infer(np.array(preprocess_for_ocr(img).convert("RGB")))
